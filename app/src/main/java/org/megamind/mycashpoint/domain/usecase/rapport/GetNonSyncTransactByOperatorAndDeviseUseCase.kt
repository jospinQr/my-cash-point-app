...
class GetNonSyncTransactByOperatorAndDeviseUseCase(private val repository: TransactionRepository) {

    operator fun invoke(
        idOperateur: Long,
        device: Constants.Devise
    ): Flow<Result<List<Transaction>>> =
        repository.getNonSyncTransactByOperatorAndDevise(idOperateur, device)
