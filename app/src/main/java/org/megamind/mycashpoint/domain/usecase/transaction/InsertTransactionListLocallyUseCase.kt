...
class InsertTransactionListLocallyUseCase(private val transactionRepository: TransactionRepository) {

    operator fun invoke(transactions: List<Transaction>): Flow<Result<Unit>> {
        return transactionRepository.insertTransactionListLocally(transactions)
    }
