...
class SendOnTransactionToBackEndUseCase(private val transactionRepository: TransactionRepository) {
    operator fun invoke(transaction: Transaction): Flow<Result<Unit>> = flow {
        transactionRepository.sendToServer(transaction).collect { emit(it) }
    }
}